// COMPILAZIONE:
// gcc client_gen_keys.c -o client_gen_keys -I/opt/homebrew/opt/openssl/include -L/opt/homebrew/opt/openssl/lib -lssl -lcrypto
...
int main() {
    ...
    FILE *private_file = fopen("client_private_key.pem", "wb");
    if (!private_file) {
    ...
    FILE *public_file = fopen("client_public_key.pem", "wb");
    if (!public_file) {
    ...
    printf("Coppia di chiavi generata e salvata nei file 'client_private_key.pem' e 'client_public_key.pem'.\n");
    return 0;
}
